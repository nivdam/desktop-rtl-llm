(() => {
  ...
  const classifier = globalThis.__LOCAL_RTL_CLASSIFIER__;
  if (!classifier) {
    console.error("[RTL] rtl-classifier.js must be evaluated before rtl-runtime.js");
    return;
  }
  const { analyzeText, detectFirstStrongDirection, isUrlOnly, isFilePathOnly, isPathLikeFragment, isJsonLikeBlock, classifyText } = classifier;
  ...
  function classifyElement(element) {
    ...
    return classifyText(text, {
      mixedRtlRatioThreshold,
      inheritsRtlContext: shouldInheritRtlContext(element),
    });
  }
